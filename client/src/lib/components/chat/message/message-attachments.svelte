<script lang="ts">
  import { attachmentUrl, type MessageAttachment, type MessageView } from "$lib/api";
  ...
  const files = $derived(message.attachments.filter((a) => !a.image));
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="text-muted-foreground text-xs">
            ...
            {#if file.expiresAt}
              &middot; expires {formatRelative(file.expiresAt)}
            {/if}
          </div>
        </div>
