...
<div class="bg-sidebar-accent/40 space-y-2 border-t p-2">
  <div class="flex items-center gap-2">
    <span
      class={cn('size-2 shrink-0 rounded-full bg-amber-500', voice.status === 'connected' && 'bg-green-500')}
    ></span>
  ...
  <div class="max-h-40 space-y-0.5 overflow-y-auto">
    {#each voice.participants as p (p.identity)}
      <div class={cn('text-muted-foreground flex items-center gap-2 rounded px-1.5 py-1 text-sm', p.speaking && 'text-foreground')}>
        <span
          class={cn('size-2 shrink-0 rounded-full bg-current opacity-50', p.speaking && 'bg-green-500 opacity-100 ring-2 ring-green-500/30')}
        ></span>
  ...
  <div class="flex gap-1.5">
    ...
    <Button variant="destructive" size="sm" onclick={() => voice.leave()}>
