...
import { boot, claim, cleanup, get, json, mkInvite, post, register, uniq } from './harness.js';
...
describe('register', () => {
  it('needs a valid invite', async () => {
    ...
    expect((await post(app, '/api/auth/register', { username: uniq(), password: 'joinpass123' })).status).toBe(400);
  });
  ...
  it('enforces the username rules', async () => {
    ...
    for (const bad of ['a', 'has spaces', 'UPPER!', 'x'.repeat(25), 'sym$bol']) {
      expect((await register(app, invite.code, bad)).status, `should reject ${bad}`).toBe(400);
    }
  });
...
describe('login', () => {
  it('accepts the right password and rejects the wrong one', async () => {
    expect((await post(app, '/api/auth/login', { username: 'owner', password: 'ownerpass123' })).status).toBe(200);
    expect((await post(app, '/api/auth/login', { username: 'owner', password: 'wrongpass123' })).status).toBe(401);
  });

  it('does not leak whether a username exists', async () => {
    const missing = await post(app, '/api/auth/login', { username: 'ghost', password: 'whatever123' });
    const wrong = await post(app, '/api/auth/login', { username: 'owner', password: 'whatever123' });
...
describe('sessions', () => {
  ...
  it('locks out a banned user', async () => {
    ...
    expect((await post(app, '/api/auth/login', { username: name, password: 'joinpass123' })).status).toBe(403);
  });
});
