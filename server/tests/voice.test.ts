...
const config = (headers: Record<string, string>) =>
  app.request('/api/voice/config', { headers: { authorization: `Bearer ${token}`, ...headers } });
...
describe('LiveKit URL derivation', () => {
  it('uses wss when the proxy terminated TLS', async () => {
    const res = await config({ 'x-forwarded-proto': 'https', 'x-forwarded-host': 'chat.example.com' });
