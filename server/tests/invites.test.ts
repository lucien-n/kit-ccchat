...
describe('revoking', () => {
  it('kills a leaked link', async () => {
    ...
    const revoked = await post(app, `/api/invites/${invite.code}/revoke`, undefined, token).then(json);
...
describe('permissions', () => {
  it('does not let a plain member mint or list invites', async () => {
    ...
    expect((await post(app, '/api/invites', { maxUses: 0 }, member.token)).status).toBe(403);
